...
def init_robots(warehouse: Warehouse):
    ...
    robots = []
    warehouse_rows, warehouse_cols = warehouse.getWarehouseSize() or [31, 49]
    x_min = min(5, max(0, warehouse_cols - 1))
    x_max = max(x_min, warehouse_cols - 6)
    y_min = 0
    y_max = max(y_min, warehouse_rows - 1)
    x_range = (x_min, x_max)
    y_range = (y_min, y_max)
...
def create_storage_object(warehouse: Warehouse, x, y, row, totalRows, value, data: DataFrame):
    ...
    if value in [0, 1, 2]:
        ...
        # Station-lane cells must be traversable for robots both with and without pods.
        warehouse.graph.addEdge(obj_key, obj_right_coordinate, weight=weight)
        warehouse.graph_pod.addEdge(obj_key, obj_right_coordinate, weight=weight)
    elif value in [13, 22]:
        warehouse.graph.addEdge(obj_key, obj_left_coordinate, weight=weight)
        ...
        warehouse.graph.addEdge(obj_key, obj_above_coordinate, weight=weight)
        ...
        warehouse.graph.addEdge(obj_key, coord, weight=weight)
