...
PRODUCT_TRAIN_OUTPUT_PATH = BASE_DIR / "preprocessed_train.csv"
PRODUCT_TEST_OUTPUT_PATH = BASE_DIR / "preprocessed_test.csv"
ORDER_TRAIN_OUTPUT_PATH = BASE_DIR / "訂單資料_train.csv"
ORDER_TEST_OUTPUT_PATH = BASE_DIR / "訂單資料_test.csv"
...
def split_preprocessed_datasets(
    product_df: pd.DataFrame,
    order_df: pd.DataFrame,
    cutoff_ratio: float = DEFAULT_CUTOFF_RATIO,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    if not 0.0 < float(cutoff_ratio) < 1.0:
        raise ValueError("cutoff_ratio must be between 0 and 1.")

    order_working = order_df.copy()
    order_working["_created_timestamp"] = pd.to_datetime(
        order_working["created_at"],
        format="%d/%m/%Y %H:%M",
        errors="coerce",
    )
    if order_working["_created_timestamp"].isna().any():
        invalid_rows = int(order_working["_created_timestamp"].isna().sum())
        raise ValueError(
            f"Cannot split order dataset because {invalid_rows} rows have invalid created_at timestamps."
        )

    min_time = order_working["_created_timestamp"].min()
    max_time = order_working["_created_timestamp"].max()
    cutoff_time = min_time if min_time == max_time else min_time + (max_time - min_time) * float(cutoff_ratio)

    train_order_df = (
        order_working[order_working["_created_timestamp"] <= cutoff_time]
        .drop(columns=["_created_timestamp"])
        .copy()
    )
    test_order_df = (
        order_working[order_working["_created_timestamp"] > cutoff_time]
    ...
    train_item_codes = set(train_order_df["item_code"].map(normalize_item_code))
    test_item_codes = set(test_order_df["item_code"].map(normalize_item_code))

    train_product_df = product_df[product_df["item_code"].isin(train_item_codes)].copy()
    test_product_df = product_df[product_df["item_code"].isin(test_item_codes)].copy()

    split_summary_df = pd.DataFrame(
        [
            {"metric": "cutoff_ratio", "value": float(cutoff_ratio)},
            {"metric": "cutoff_timestamp", "value": str(cutoff_time)},
            {"metric": "train_order_rows", "value": int(len(train_order_df))},
            {"metric": "test_order_rows", "value": int(len(test_order_df))},
            {"metric": "train_unique_orders", "value": int(train_order_df["order_id"].nunique())},
            {"metric": "test_unique_orders", "value": int(test_order_df["order_id"].nunique())},
            {"metric": "train_unique_item_codes", "value": int(train_order_df["item_code"].nunique())},
            {"metric": "test_unique_item_codes", "value": int(test_order_df["item_code"].nunique())},
            {"metric": "train_product_rows", "value": int(len(train_product_df))},
            {"metric": "test_product_rows", "value": int(len(test_product_df))},
            {"metric": "train_product_unique_item_codes", "value": int(train_product_df["item_code"].nunique())},
            {"metric": "test_product_unique_item_codes", "value": int(test_product_df["item_code"].nunique())},
            {
                "metric": "product_overlap_item_codes",
                "value": int(len(set(train_product_df["item_code"]) & set(test_product_df["item_code"]))),
            },
        ]
    )

    return train_product_df, test_product_df, train_order_df, test_order_df, split_summary_df
...
def main() -> None:
    ...
    product_train_output_path = save_dataframe(
        product_train_df,
        PRODUCT_TRAIN_OUTPUT_PATH,
        index=False,
        encoding="utf-8-sig",
        sep=";",
        decimal=",",
    )
    product_test_output_path = save_dataframe(
        product_test_df,
        PRODUCT_TEST_OUTPUT_PATH,
        index=False,
        encoding="utf-8-sig",
        sep=";",
        decimal=",",
    )
    order_train_output_path = save_dataframe(
        order_train_df,
        ORDER_TRAIN_OUTPUT_PATH,
        index=False,
        encoding="utf-8-sig",
        sep=";",
        decimal=",",
    )
    order_test_output_path = save_dataframe(
        order_test_df,
        ORDER_TEST_OUTPUT_PATH,
        index=False,
        encoding="utf-8-sig",
        sep=";",
        decimal=",",
    ...
    split_summary_output_path = save_dataframe(
        split_summary_df,
        SPLIT_SUMMARY_PATH,
        index=False,
        encoding="utf-8-sig",
    ...
    print(f"Order test CSV:        {safe_console_text(order_test_output_path)}")
