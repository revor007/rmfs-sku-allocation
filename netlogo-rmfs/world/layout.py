...
class Layout(object):
    def __init__(self, total_pods_active=472, total_charging_stations=10, random_seed=42):
        ...
        self.total_charging_stations = int(total_charging_stations)
        self.random_seed = int(random_seed)
        self.configure_capacity()

    def podCellsPerVerticalBand(self):
        return (
            self.pod_batch_horizontal
            * self.pod_batch_horizontal_max
            * self.pod_batch_vertical
        )
    ...
    def adjustPodAvailability(self, matrix):
        ...
        required_total_positions = self.total_pods_active + self.total_charging_stations
        if current_total_pods < required_total_positions:
            raise ValueError(
                f"Generated layout only has {current_total_pods} raw pod cells, "
                f"but requires at least {required_total_positions}."
            )

        pods_to_deactivate = current_total_pods - required_total_positions
        ...
        if pods_to_deactivate > 0:
            for r, c in rng.sample(pod_positions, pods_to_deactivate):
        ...
        if pods_to_convert > 0:
            for r, c in rng.sample(pod_positions, pods_to_convert):
